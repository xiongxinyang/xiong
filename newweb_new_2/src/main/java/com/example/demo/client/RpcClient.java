...
public class RpcClient {
   ...
   private  RpcClient(ManagedChannel channel) {
    ...
    //指令下发（立即拍照）
    @SneakyThrows
    public final void takePhotoCommand(String serverid, int devicetype, String deviceid, JttProtocol.CommandId commandId, JttProtocol.TakePhotoCommandBody takePhotoCommandBody, StreamObserver<JttProtocol.Respone> streamObserver){
        JttRequestGrpc.JttRequestStub stub = JttRequestGrpc.newStub(channel);
        JttProtocol.BaseRequest jtt = JttProtocol.BaseRequest.newBuilder()
                .setDeviceId(deviceid)
                .setDeviceType(devicetype)
                .setServiceId(serverid).build();
        JttProtocol.DownRequest dev= JttProtocol.DownRequest.newBuilder()
                .setBaseRequest(jtt)
                .setCommandId(commandId)
                .setTakePhotoCommandBody(takePhotoCommandBody).build();


        stub.takePhotoCommand(dev, streamObserver);
    }


    //指令下发（终端参数设置）
    @SneakyThrows
    public final void terminalControlCommand(String serverid, int devicetype, String deviceid, JttProtocol.CommandId commandId, JttProtocol.TerminalControlCommandBody terminalControlCommandBody, StreamObserver<JttProtocol.Respone> streamObserver){
        JttRequestGrpc.JttRequestStub stub = JttRequestGrpc.newStub(channel);
        JttProtocol.BaseRequest jtt = JttProtocol.BaseRequest.newBuilder()
                .setDeviceId(deviceid)
                .setDeviceType(devicetype)
        ...
        JttProtocol.DownRequest dev= JttProtocol.DownRequest.newBuilder()
                .setBaseRequest(jtt)
                .setCommandId(commandId)
                .setTerminalControlCommandBody(terminalControlCommandBody).build();


        stub.terminalControlCommand(dev, streamObserver);
    }
